...
from .. import errors
from ..utils.config import PACKAGE_TYPES, UPLOAD_TYPE_MAPPING
    ...
    if args.package_type:
        ...
        if package_type is None:
            message = (
                ...
                "type with option -t or --package-type (Incase of General Artifact, use -t or --package-type gra)"
                % filename
            )
...
class SubCommand(SubCommandBase):
        ...
        for filepath in self.args.files:
            for fp in filepath:
                for channel in channels:
                    ...
                    if package_type == UPLOAD_TYPE_MAPPING["gra"]:
                        if not self.args.name:
                            message = "Name is required for General Artifact. Please provide name with -n or --name"
                            raise errors.RepoCLIError(message)
                    resp = self.api.upload_file(
                        fp, channel, package_type, self.args.name, self.args.version
                    )
    ...
    def add_parser(self, subparsers):
        ...
        graoptions = parser.add_argument_group("General Artifact Upload options")
        graoptions.add_argument(
            "-n",
            "--name",
            help="The file will be Grouped under the provided name (The name is required for General Artifact)",
        )

        graoptions.add_argument(
            "-v",
            "--version",
            help="Set the version of the General Artifact",
        )
